<template>
    ...
    > -->
        ...
        <div class="absolute top-12 right-12 w-20 h-20 rounded-full bg-white/30"></div>
        <div class="absolute bottom-12 left-12 w-32 h-32 rounded-full bg-white/30"></div>

        <!-- Message Content -->
        <div class="flex-1 flex flex-col items-center p-6 justify-center">
            <!-- Text Message -->
            <p v-if="message.message_type === 'text'" class="text-center font-light"
                style="font-size: 72px; line-height: 1.4">
                {{ message.text }}
            </p>

            <!-- Image Message -->
            <div v-else-if="message.message_type === 'image'" class="flex flex-col items-center gap-4">
                <p v-if="message.text" class="text-center font-light" style="font-size: 48px; line-height: 1.4">
                    {{ message.text }}
                </p>
            ...
            <!-- Video Message -->
            <div v-else-if="message.message_type === 'video'" class="flex flex-col items-center gap-4">
                <p v-if="message.text" class="text-center font-light" style="font-size: 48px; line-height: 1.4">
                    {{ message.text }}
                </p>
            ...
            <!-- Audio Message -->
            <div v-else-if="message.message_type === 'audio'" class="flex flex-col items-center gap-4">
                <p v-if="message.text" class="text-center font-light" style="font-size: 48px; line-height: 1.4">
                    {{ message.text }}
                </p>
                <div v-else class="text-center font-light" style="font-size: 48px; line-height: 1.4">
                ...
                <div class="relative w-[800px] h-[200px] bg-transparent flex items-center justify-center">
                    <div class="w-full h-full flex items-center justify-center rounded-lg">
        ...
        <div class="mt-auto pt-6">
            ...
            <p class="text-center opacity-90 font-light" style="font-size: 52px">
                @{{ username }}
            </p>
        </div>
        <div class="mt-32 flex flex-col items-center">
            <div class="mb-2 rounded-full p-6 flex items-center justify-center">
                <img src="/images/logo_icon_colored.png" alt="Incognito logo" class="w-[100px] h-[100px]" />
            </div>
            <!-- <UIcon name="i-custom-logo-text" size="40px" class="text-white" /> -->
            <p class="font-bold tracking-wider" style="font-size: 40px">
                #INCOGNITO
            </p>
